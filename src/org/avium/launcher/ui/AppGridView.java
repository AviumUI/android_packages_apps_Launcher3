...
public class AppGridView extends View {
    ...
    private static final float ITEM_HEIGHT_DP = 45f;
    ...
    public int getIndexAt(float x, float y) {
        ...
        int col;
        if (mIsLeftEdge) {
            col = (int) (x / mItemWidth);
        } else {
            col = (int) ((getWidth() - x) / mItemWidth);
        }
        int row = (int) (y / mItemHeight);
        int index = row * mAppsPerRow + col;
        if (col < 0 || col >= mAppsPerRow || y < 0 || y > mTotalContentHeight) {
            return -1;
        }
        return Math.max(0, Math.min(index, mApps.size() - 1));
    }
    ...
    private void updatePosition() {
        ...
        int newTop = (int) (mAnchorY - height / 2 - 15);
