...
public class AppIndexOverlay extends FrameLayout {
    ...
    private void init(Context context) {
        ...
        gridParams.gravity = Gravity.LEFT | Gravity.CENTER_VERTICAL;
    ...
    public void setEdgeSide(boolean isLeftEdge) {
        ...
        if (isLeftEdge) {
            ...
            gridParams.gravity = Gravity.RIGHT | Gravity.CENTER_VERTICAL;
    ...
    private void updateLetterIndicatorPosition() {
        ...
        float letterY = selectedIndex >= 0 ? mLetterBar.getLetterCenterY(selectedIndex) : mAppGridHeight / 2;
        params.topMargin = (int) (letterY - indicatorSize - 14 * density);
    ...
    public boolean isInLetterArea(float x) {
        if (mIsLeftEdge) {
            return x < mLetterBarWidth + mSpacing / 2;
        } else {
            return x > getWidth() - mLetterBarWidth - mSpacing / 2;
        }
    }

    public boolean isInAppArea(float x, float y) {
        int gridLeft = mIsLeftEdge ? mLetterBarWidth + mSpacing : getWidth() - mLetterBarWidth - mSpacing - mAppGridWidth;
        int gridRight = gridLeft + mAppGridWidth;
        int gridTop = mAppGrid.getTop();
        int gridBottom = gridTop + mAppGridHeight;
        return x >= gridLeft && x <= gridRight && y >= gridTop && y <= gridBottom;
    }

    public int getLetterIndexAtY(float y) {
        return mLetterBar.getIndexAtY(y);
    }
